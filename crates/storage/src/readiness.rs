...
use sqlx::PgPool;
use readiness::{model::DeploymentReadinessReport, repository::{ReadinessReportRepository, RepositoryError}};
use types::ReleaseId;
use uuid::Uuid;

pub struct SqlxReadinessRepository { pool: PgPool }
impl SqlxReadinessRepository { pub fn new(pool: PgPool) -> Self { Self { pool } } }

#[derive(sqlx::FromRow)]
struct Row { release_id: Uuid, data: serde_json::Value }

#[async_trait]
impl ReadinessReportRepository for SqlxReadinessRepository {
    async fn save(&self, report: &DeploymentReadinessReport) -> Result<(), RepositoryError> {
        ...
        sqlx::query("INSERT INTO readiness_reports (release_id, data) VALUES ($1, $2) ON CONFLICT (release_id) DO UPDATE SET data = $2")
            .bind(rel_id)
            .bind(data)
    ...
    async fn find_by_release_id(&self, release_id: ReleaseId) -> Result<Option<DeploymentReadinessReport>, RepositoryError> {
        ...
        let row = sqlx::query_as::<_, Row>("SELECT release_id, data FROM readiness_reports WHERE release_id = $1")
            .bind(rel_id)
